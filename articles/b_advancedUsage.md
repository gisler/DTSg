...
[`clone()`](https://gisler.github.io/DTSg/reference/clone.DTSg.md)
...
[`new()`](https://gisler.github.io/DTSg/reference/DTSg.md) method. When
set to `TRUE`, a `data.table` provided through the `values` argument is
“swallowed” by the `DTSg` object, i.e. no copy of it is made and all
references to it are removed from the global (and only the global)
environment upon successful object creation:
