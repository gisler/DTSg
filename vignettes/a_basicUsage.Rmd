...
Additional temporal aggregation level functions exist for years, days, hours, minutes and seconds. Furthermore, the temporal aggregation level of certain temporal aggregation level functions can be adjusted via the `multiplier` argument of the `aggregate` as well as other methods, e.g. a `multiplier` of 6 makes `byYm____` aggregate to half years instead of months.
